...
class ContinuousprintPlugin(
	...
	def bed_cooldown(self):
		...
		timeout = time.time() + 60 * float(self._settings.get(["bed_cooldown_timeout"]))  # timeout converted to seconds
		...
		while bed_temp > float(self._settings.get(["bed_cooldown_threshold"])):
			self._logger.info(f"Bed temp at {bed_temp} waiting for {self._settings.get(['bed_cooldown_threshold'])}")
			time.sleep(5)
			bed_temp = self._printer.get_current_temperatures()['bed']['actual']
	...
	def set_active(self):
		...
		self.d.set_active(flask.request.form["active"] == "true",
